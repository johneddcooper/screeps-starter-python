...
# END predef_aliases
    ...
    def visit_FunctionDef (self, node, _async = False):
        def emitScopedBody ():
            ...
            if self.getScope (ast.AsyncFunctionDef if _async else ast.FunctionDef) .containsYield:
        ...
        if not node.name == '__pragma__':   # Don't generate code for the dummy pragma definition starting the extraLines in utils
            ...
            if type (self.getScope () .node) in (ast.Module, ast.FunctionDef, ast.AsyncFunctionDef):  # Global or function scope, so it's no method
                ...
                self.adaptLineNrString (node)
                self.emit ('var {} = {}function', self.filterId (node.name), 'async ' if _async else '')
                ...
                if decorate:
                    ...
                    self.emit ('{}: {}function', implName, 'async ' if _async else '')
                else:
                    if isClassMethod:
                        self.emit ('get {} () {{return __getcm__ (this, {}function', self.filterId (node.name),  'async ' if _async else '')
                    elif isStaticMethod:
                        self.emit ('get {} () {{return {}function', self.filterId (node.name), 'async ' if _async else '')
                    else:
                        self.emit ('get {} () {{return __get__ (this, {}function', self.filterId (node.name),  'async ' if _async else '')
